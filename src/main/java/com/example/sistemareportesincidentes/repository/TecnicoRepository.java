...
//
//    @Query("SELECT i.tecnicoAsignado, COUNT(i) as incidentesResueltos FROM Incidente i JOIN i.detalles d JOIN d.tipoProblema tp JOIN tp.especialidadesRequeridas e WHERE i.estado = 'RESUELTO' AND i.fechaResolucion >= :fechaInicio AND e.idEspecialidad = :especialidadId GROUP BY i.tecnicoAsignado ORDER BY incidentesResueltos DESC")
